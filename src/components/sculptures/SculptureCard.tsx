...
export default function SculptureCard({
  ...
  return (
    <Link href={`/sculptures/${slug}`} className="group block h-full">
      <article className="relative flex h-full flex-col overflow-hidden rounded-[28px] border border-black/[0.06] bg-white/82 shadow-[0_8px_32px_rgba(0,0,0,0.055)] backdrop-blur-sm transition-all duration-500 hover:-translate-y-2 hover:border-[#c8873f]/18 hover:shadow-[0_24px_64px_rgba(0,0,0,0.10)]">
        {/* Top shine on hover */}
        ...
        <div className="flex flex-1 flex-col p-5">
          ...
          <h2 className="mt-2 line-clamp-2 text-[1.08rem] font-semibold leading-[1.2] tracking-[-0.02em] text-neutral-900 transition duration-300 group-hover:text-[#8b4d1a]">
            {title}
          </h2>

          {subtitle ? (
            <p className="mt-1.5 line-clamp-2 text-[12px] leading-[1.55] text-neutral-500">
              {subtitle}
            </p>
          ) : null}
          ...
          <div className="mt-auto flex items-center justify-between border-t border-black/[0.06] pt-4">
            {dimensions || year ? (
              ...
              <p className="text-[11px] font-medium text-[#9b6a35]">
                {t("discover")}
              </p>
            )}

            <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-[#f3ece4] text-[#9b6a35] ring-1 ring-[#e4d6c1]/60 transition-all duration-300 group-hover:bg-[linear-gradient(135deg,#ff9f43,#e76f16,#c85100)] group-hover:text-white group-hover:ring-transparent group-hover:shadow-[0_6px_20px_rgba(200,112,56,0.35)]">
